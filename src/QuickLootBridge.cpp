...
namespace QuickPocket
{
	...
	{
		const auto ready = QuickLoot::API::QuickLootAPI::Init(PluginName, QuickLoot::API::ApiVersion::kV20);
		logger::info(
			"[QP] QuickLoot bridge initialized: v20={} v21={}",
			QuickLoot::API::QuickLootAPI::IsReady(QuickLoot::API::ApiVersion::kV20) ? "yes" : "no",
			QuickLoot::API::QuickLootAPI::IsReady(QuickLoot::API::ApiVersion::kV21) ? "yes" : "no");
		if (ready && !IsCompatible()) {
			logger::info("[QP] QuickLoot IE API v21 missing; plant toggle via Take All needs 4.1.0+.");
		}
		return ready;
	}
